...
- **Usage**：Claude / Codex **各占独立一格**，占位数据；接口 `UsageProvider`（`lib/usage.ts`）已约定好，EVO-77 就绪后替换 `usageProvider` 一行即可。
...
- **Quick Links**（取代书签）：用户可编辑列表（增/删/改 + 排序），存 `chrome.storage.local`（localStorage 兜底），favicon 自动取。**不再读 `chrome.bookmarks`，已移除 `bookmarks` 权限。**
